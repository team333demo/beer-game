...
function checkstat($period) {
	...
	$sql = "select wstat from factory where period = (?-1)";
	$stmt = mysqli_prepare($db, $sql);
	mysqli_stmt_bind_param($stmt, "i",$period);
    mysqli_stmt_execute($stmt); //執行SQL
    $result = mysqli_stmt_get_result($stmt);
	...
	$sql = "select fstat from distributor where period = ?";
	$stmt = mysqli_prepare($db, $sql);
	mysqli_stmt_bind_param($stmt, "i",$period);
    mysqli_stmt_execute($stmt); //執行SQL
    $result = mysqli_stmt_get_result($stmt);
...
function countstock ($period) {
	global $db;
	$sql="select stock as laststock from factory where period=?";
	$stmt = mysqli_prepare($db, $sql);
	mysqli_stmt_bind_param($stmt, "i",$period);
    mysqli_stmt_execute($stmt); //執行SQL
    $result = mysqli_fetch_assoc(mysqli_stmt_get_result($stmt)); 
	...
	$sql="select distributor.dord as ord from distributor where period=(?+1)";
	$stmt = mysqli_prepare($db, $sql);
	mysqli_stmt_bind_param($stmt,"i",$period);
    mysqli_stmt_execute($stmt); //執行SQL
    $result = mysqli_fetch_assoc(mysqli_stmt_get_result($stmt)); 
	...
	$sql="select arrival as arrive from factory where period=(?+1)";
	$stmt = mysqli_prepare($db, $sql);
	mysqli_stmt_bind_param($stmt, "i",$period);
    mysqli_stmt_execute($stmt); //執行SQL
    $result = mysqli_fetch_assoc(mysqli_stmt_get_result($stmt)); 
...
function updatestock ($stock,$period) {
	global $db;
	$sql="update factory set stock = ? where period = ?";
	$stmt = mysqli_prepare($db, $sql);
...
function countcost ($period) {    //成本
	global $db;
    $sql= "select stock  from factory where period = ?";
	$stmt = mysqli_prepare($db, $sql);
	mysqli_stmt_bind_param($stmt,"i",$period);
    mysqli_stmt_execute($stmt); //執行SQL
    $result = mysqli_stmt_get_result($stmt); 
	...
	$sql = "update factory set cost = ? where period = ?";
    $stmt = mysqli_prepare($db, $sql);
...
function updatearrival($period) { // 修改到貨量
    global $db; 
    $sql = "select ford as arr from factory where period = (?-2)";
    $stmt = mysqli_prepare($db, $sql); 
    mysqli_stmt_bind_param($stmt, "i", $period);
    mysqli_stmt_execute($stmt); //執行SQL
    $result = mysqli_stmt_get_result($stmt);
    ...
    if ($period >= 2) {
        $sql = "update factory set arrival = ? where period = ?";
        $stmt = mysqli_prepare($db, $sql);
        mysqli_stmt_bind_param($stmt, "ii", $arr['arr'],$period);
    } else {
        $sql = "update factory set arrival = 0 where period = ?";
        $stmt = mysqli_prepare($db, $sql);
        mysqli_stmt_bind_param($stmt, "i",$period);
    }
    mysqli_stmt_execute($stmt); 
...
function updatesales($period) { // 修改銷貨量
    global $db; 
    $sql = "select stock, distributor.dord , arrival from distributor,factory where period = ?";
    $stmt = mysqli_prepare($db, $sql);
    mysqli_stmt_bind_param($stmt, "i",$period);
    mysqli_stmt_execute($stmt); //執行SQL
    $result = mysqli_stmt_get_result($stmt);
    ...
    $sql2 = "select stock from factory where period = ?";
    $p = $period - 1;	
    ...
    $sql = "update factory set fsale = ? where period = ?";
    $stmt = mysqli_prepare($db, $sql);
