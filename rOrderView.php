...
function insertfirst($Tid){ //新增一行 空白資料
 global $db; 
 $sql ="INSERT INTO retailer(`Tid`,`rord`,`period`,`stock`,`arrival`,`cost`,`rstat`,rsale) values (?,0,0,15,0,0,1,0);" ; 
 $stmt = mysqli_prepare($db, $sql);
 mysqli_stmt_bind_param($stmt, "i",$Tid);
 mysqli_stmt_execute($stmt);
 
 $sql ="INSERT INTO retailer(`Tid`,`rord`,`period`,`stock`,`arrival`,`cost`,`rstat`,rsale) values (?,0,1,15,0,15,0,0);" ; 
 $stmt = mysqli_prepare($db, $sql);
 mysqli_stmt_bind_param($stmt, "i",$Tid);
 mysqli_stmt_execute($stmt);
