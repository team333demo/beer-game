...
function orderlist($Tid){
	global $db;
	$sql = "select *,demand.period from `retailer`,demand where retailer.period = demand.period and retailer.Tid=?";
	$stmt = mysqli_prepare($db, $sql );
...
function period() {
    global $db;
    $sql = "select max(period) as currPeriod from retailer";
    $stmt = mysqli_prepare($db, $sql);
    //mysqli_stmt_bind_param($stmt, "i",$Tid);
...
function Tid(){
	global $db;
	$sql = "select retailer.Tid from `retailer`";
	$stmt = mysqli_prepare($db, $sql );
	mysqli_stmt_execute($stmt);
	$result = mysqli_stmt_get_result($stmt);
	return $result;
